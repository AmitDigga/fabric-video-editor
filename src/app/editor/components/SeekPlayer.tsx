...
export const SeekPlayer = (props:Props)=>{
    ...
    return <div className="seek-player flex flex-col">
        ...
        <button className="w-[80px] rounded bg-black text-white px-2 py-2" onClick={()=>{
            if(play) props.onPause();
            else props.onPlay();
            return setPlay(!play);
        }}>{play ? "Pause" : "Play"}</button>
        <span className="w-[50px] ml-2">{Math.floor(currentTime/1000 * 10)/10}s</span>
        <span className="w-[50px]">/ {Math.floor(maxTime/1000 * 10)/10}s</span>
        </div>
